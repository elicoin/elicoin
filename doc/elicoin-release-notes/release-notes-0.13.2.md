Elicoin Core version 0.13.2 is now available from:

  <https://download.elicoin.org/elicoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/elicoin-project/elicoin/issues>
...
updates will be released anymore. Without security updates, using a elicoin
...
ECDSA signatures inside Elicoin transactions now use validation using
...
Previous versions of Elicoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Elicoin Core was to raise the default minimum
relay fee.

Elicoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Elicoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running elicoind without having to do any manual configuration.
...
Elicoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Elicoin Core will relay transactions with
...
entirely in Elicoin Core 0.13, and it is at this time undecided whether the
...
Elicoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Elicoin Core automatically creates a hidden service to listen on, without
manual configuration. Elicoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Elicoin Core is listening, and
...
Elicoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Elicoin
Core to use floating fees. By default, Elicoin Core will use floating
fees.
...
(default: `0.0002` ELI/kB).

At all times, Elicoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ELI.
Furthermore, Elicoin Core will never create transactions paying less than
...
`elicoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /elicoinrpc>
        ...
        # optional bypass elicoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< elicoinrpc:<password>
    </Location>
...
Restarting elicoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in elicoin.conf
...
For example:
    ...
    $ src/elicoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Elicoin Core code base has
...
- `elicoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `elicoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Elicoin scripting system that in combination with
...
Elicoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Elicoin Core.
...
The two phases are distinct in the Elicoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Elicoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Elicoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Elicoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of elicoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of elicoins being spent is exactly
...
- **Increased security for multisig:** Elicoin addresses (both P2PKH addresses
  ...
  believed to be provided by Elicoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Elicoin's current ledger (called the UTXO set),
  ...
  note that Elicoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Elicoin users to individually opt-in to almost any change in the Elicoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Elicoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Elicoin
client to Elicoin Core 0.13.2 to signal for segwit support. When a
...
Since Elicoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `elicoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `elicoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
