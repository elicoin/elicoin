...
*Setup instructions for a Gitian build of Rtidcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Rtidcoin
...
- [Building Rtidcoin Core](#building-rtidcoin-core)
...
Clone the git repositories for rtidcoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/rtidcoin-project/rtidcoin
...
in the rtidcoin repository under 'Fetch and create inputs' to install sources which require
...
To build Rtidcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the rtidcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/rtidcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/rtidcoin-project/rtidcoin
...
For example:
```bash
URL=https://github.com/thrasher-/rtidcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit rtidcoin=${COMMIT} --url rtidcoin=${URL} ../rtidcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit rtidcoin=${COMMIT} --url rtidcoin=${URL} ../rtidcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit rtidcoin=${COMMIT} --url rtidcoin=${URL} ../rtidcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the rtidcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../rtidcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/rtidcoin
SIGPATH=/some/root/path/rtidcoin-detached-sigs

./bin/gbuild --url rtidcoin=${BTCPATH},signature=${SIGPATH} ../rtidcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/rtidcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/rtidcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/rtidcoin-osx-build.assert
```
...
[rtidcoin-project/gitian.sigs.ltc](https://github.com/rtidcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
